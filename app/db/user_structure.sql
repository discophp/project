...
CREATE TABLE `user_login_token` (
  ...
  CONSTRAINT `ult_user_id` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `user_token` (
  ...
  `token` char(172) NOT NULL,
  `created_on_date` datetime NOT NULL,
  `expires_on_date` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `ut_user_id_idx` (`user_id`),
  CONSTRAINT `ut_user_id` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;
